...
import ParametrosForm from "./frmRParametro";
...
const GuardarLiquidaciones = ({ onClose }) => {
    ...
    const [isNuevoFormularioOpen, setIsNuevoFormularioOpen] = useState(false);
    ...
    const { getParametro, parametros, deleteParametro } = useParametro();
    const sEstado = [
        "ACTIVO",
        "INACTIVO"
    ];
    ...
    const handleDeleteParametro = (val) => {
        toast(
            (t) => (
                <div style={{ textAlign: "center", fontWeight: "bold" }}>
                    <p>¿Realmente desea eliminar el parametro</p>
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '10px' }}>
                        <button className="toast-button-confirmed" onClick={() => {
                            deleteParametro(val.idParametro)
                                .then(() => {
                                    toast.dismiss(t.id);
                                    toast.success(<b>El parametro fue eliminado exitosamente!</b>);
                                    getUsuario();
                                })
                                .catch((error) => {
                                    toast.error(<b>Error: {error.response.data.message}</b>);
    ...
    const setParametro = (val) => {
        setEditar(true);
        setParametroEdit(val);
        setIsNuevoFormularioOpen(true);
    };
    ...
    const handleNuevoFormularioOpen = () => {
        setParametroEdit(null);
        setIsNuevoFormularioOpen(true);
    };

    const handleNuevoFormularioClose = () => {
        setIsNuevoFormularioOpen(false);
        setParametroEdit(null);
    };
    ...
    return (
        <div className="w-full h-full">
            <Toaster />
            {isNuevoFormularioOpen ? (
                <ParametrosForm onClose={handleNuevoFormularioClose} parametroEdit={parametroEdit} 
                isReadOnly={false} />
            ) : (
                // Si no está abierto el formulario de parámetros, entonces muestra los otros formularios o la tabla
                isFormOpen ? (
                    isLiquidarFormOpen ? (
                        <LiquidarEmpleadoForm
                        ...
                        <div className="table-card-empleados">
                            ...
                            <div className="button-container">
                                <button
                                    ...
                                    onClick={handleNuevoFormularioOpen}
                                >
                                    Parametros
                                </button>
                            </div>
                            <div className="table-card-empleados">
                                <div className="table-card-empleados">
                                    <h1 className="sub-titles-copm">Parametros Registrados</h1>
                                    <table>
                                        <thead>
                                            <tr>
                                            ...
                                            {parametros.map((val, key) => (
                                                <tr key={val.idParametro}>
                                                    ...
                                                    <td>
                                                        <button
                                                            className="edit-button"
                                                            onClick={(e) => { e.stopPropagation(); setParametro(val, false); }}
                                                        >
                                                            <i className="fi fi-br-customize-edit icon-style-components"></i>
                                                        </button>
                                                        <button
                                                            className="delete-button"
                                                            onClick={(e) => { e.stopPropagation(); handleDeleteParametro(val); }}
                                                        >
                                                            <i className="fi fi-br-clear-alt icon-style-components"></i>
                                                        </button>
                                                    </td>
